...
export default function AuthScreen({ onAuth }: Props) {
  ...
  const handleGoogle = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        // With implicit flow, Supabase puts the token in the URL hash on redirect
        // No callback route needed — just redirect back to the home page
  ...
  const handleWhyPm = async () => {
    ...
    if (data.user) {
      await supabase.from('profiles').upsert({ id: data.user.id, name, why_pm: whyPm })
    }
        ...
        {step === 'credentials' && (
          <div className="animate-fade-up">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-100 text-gray-900 font-medium py-3 rounded-xl text-sm transition-colors mb-5 disabled:opacity-60"
            >
              {googleLoading
              ...
              {mode === 'signup' && (
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full bg-ink-900 border border-ink-800 text-white placeholder-gray-600 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent/60"
                  />
                </div>
              )}
              <div>
              ...
              {WHY_PM_OPTIONS.map((opt) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-300 leading-relaxed">{opt.detail}</div>
                  </div>
                  {whyPm === opt.id && (
